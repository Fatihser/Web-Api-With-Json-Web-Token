	...
	public class CategoryController : Controller
	{
		...
		public async Task<IActionResult> List()
		{
			...
			if (token != null)
			{
				...
				var response = await client.GetAsync("http://localhost:5149/api/categories");
		...
		public async Task<IActionResult> Remove(int id)
		{
			...
			if (token != null)
			{
				...
				await client.DeleteAsync($"http://localhost:5149/api/categories/{id}");
			}
			...
			if (ModelState.IsValid)
			{
				...
				if (token != null)
				{
					...
					var response = await client.PostAsync(
						"http://localhost:5149/api/categories", content);
		...
		public async Task<IActionResult> Update(int id)
		{
			...
			if (token != null)
			{
				...
				var response = await client.GetAsync(
					$"http://localhost:5149/api/categories/{id}");
				if (response.IsSuccessStatusCode)
				{
					var jsonData = await response.Content.ReadAsStringAsync();
